...
namespace SignalSources.Twitter
{
    public class TwitterConnection: ISignalSourceProvider
    {
        ...
        public async Task<IEnumerable<ISignal>> GetSignalsAsync(string profileName, DateTimeOffset publishedAfter)
        {
            List<ISignal> ret = new();
            var timeline = await this.userClient.Timelines.GetUserTimelineAsync(profileName);
            foreach (var tweet in timeline.Where(x => x.CreatedAt >= publishedAfter))
            {
                var hashTags = tweet.Hashtags.Select(x => x.Text).ToList();
                ret.Add(new TwitterSignal(tweet.CreatedAt.DateTime, tweet.Text, SignalLevel.Mid, tweet.CreatedBy.Name, hashTags, tweet.Url));
            }
            return ret;
        }
    }
